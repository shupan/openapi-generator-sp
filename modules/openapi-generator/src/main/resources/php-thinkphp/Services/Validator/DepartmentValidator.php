...
namespace Modules\Department\Services\Validator;
...
class DepartmentValidator extends BaseService
{
...
//            'createDeptGroup' => 'required',
        ...
        $departmentCreateRequestArr = ArrayHelper::object2Array($departmentCreateRequest);
        try {
            $this->validateParam($departmentCreateRequestArr, $validate);
        } catch (\Exception $e) {
            throw new DepartmentException($e->getMessage());
        }

        // 检查名称是否重复
        $department = DepartmentModel::where('name', $departmentCreateRequest->name)->first();
        if (!empty($department)) {
...
//        $request = $departmentCreateRequestArr;
...
//        $this->validateParam($request, ['userPermits' => 'string', 'max:200']);
    ...
    public
    function updateDepartmentValidator(array $departmentUpdateRequest)
    {
        $validate = [
    ...
    /**
     * @param DepartmentService $departmentGetRequest
     * @return void
     * @throws DepartmentException
     */
    public function getDepartmentValidator(DepartmentGetRequest $departmentGetRequest)
    {
        $validate = [
            'id' => 'required',
        ];
        try {
            $this->validateParam(ArrayHelper::object2Array($departmentGetRequest), $validate);
        } catch (\Exception $e) {
            throw new DepartmentException($e->getMessage());
        }
    }
